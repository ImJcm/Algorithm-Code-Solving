...
1. 모든 섬의 연결이 이루어지기 때문에 가장 적은 비용의 다리를 우선적으로 선택하여 모든 섬을 연결해야 한다.
2. 이때, 섬의 비용을 기준으로 오름차순 정렬하는 우선순위 큐를 사용하여 다리를 선택한다.
3. 0번 섬을 시작으로 0을 출발지로 최소 비용을 선택하여 다음 섬과 연결된 다리들을 우선순위 큐에 추가하여 반복한다.
4. 모든 섬이 도달되면 비용을 ans에 업데이트한다.
 */
public class 섬_연결하기 {
    static void main() {
        int n = 4;
