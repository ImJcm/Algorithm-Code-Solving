...
/*
알고리즘 핵심
1. 백트랙킹으로 모든 괄호를 포함하거나 포함하지 않는 연산식을 만든다.
2. 1번 과정을 거쳐 만들어진 연산식을 대상으로 괄호를 우선 계산하여 새로운 연산식을 만든다.
3. 2번 과정을 거쳐 만들어진 연산식을 대상으로 * 연산을 우선 계산한 새로운 연산식을 만든다.
4. +,-의 연산 우선순위는 같으므로 왼쪽부터 최종 연산을 수행하여 ans에 최대값을 업데이트한다.
 */
