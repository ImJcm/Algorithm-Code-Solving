...
처음 접근으로 요구사항대로 로직을 완성하였으나 폭발 문자열을 지우고 새로운 문자열을 구성할 때 만들어진 폭발 문자열을 지우지 못했다.
두번째 접근으로 시간 초과가 발생하여 어떤 부분에서 문제가 발생하였는지 감이 잡히지 않았다.
세번째 접근으로 stack에서 문자열을 지우고 폭발 문자열이 아닌 경우 다시 넣는 과정이 시간 초과가 발생할 것이라고 예상하여
이 과정에서 stack 대신 List를 이어붙이는 것으로 개선하였지만 메모리 초과가 발생하였다.
결국 Stack 또는 StringBuilder를 사용하여 최적화하는 방법을 사용하기로 하였다.
시간초과가 발생한 부분은 폭발 문자열을 제거한 최종 문자열을 담는 과정에서 StringBuilder의 insert가 O(N)이 발생할 수 있다는
